...
class CourseBlockRecord(SparseRecord):
    ...
    local_block_id = StringField(length=255, nullable=False, description='Local block identifier (without course or type).')

    # Calculated values:
    depth = IntegerField(nullable=True, description='One-based tree depth')
    order_index = IntegerField(
        nullable=True,
        description='Number indicating the position that this block holds in a course-outline sorted list of blocks.'
    )
    extra_parents = IntegerField(nullable=True, description='The number of extra parents of this block in course DAG.')
    detached_children = IntegerField(nullable=True, description='The number of children of this block which were assigned other parents.')
    ...
    # Additional metadata:
    block_format = StringField(length=255, nullable=True, description='blah.')  # originally 'format'
    is_graded = BooleanField(nullable=True, description='True if the block is marked to be graded.')
    is_student_view_multi_device = BooleanField(nullable=True, description='blah.')
    ...
    is_visible_to_staff_only = BooleanField(nullable=True, description='blah.')
    show_correctness = StringField(length=255, nullable=True, description='blah.')
    due = DateTimeField(nullable=True, description='blah.')
    ...
    # (block_type=video)
    duration = FloatField(nullable=True, description='blah.')
    only_on_web = BooleanField(nullable=True, description='blah.')
    youtube_video_url = StringField(length=255, nullable=True, description='blah.')
    mobile_low_video_url = StringField(length=255, nullable=True, description='blah.')
    mobile_high_video_url = StringField(length=255, nullable=True, description='blah.')
    hls_video_url = StringField(length=255, nullable=True, description='blah.')
    fallback_video_url = StringField(length=255, nullable=True, description='blah.')
